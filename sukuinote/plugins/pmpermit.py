...
from ..database.pmpermit import get_authorized, AuthorizedUsers
...
async def pm_block(client, message):
	...
	async with lock:
		auth = get_authorized(message.from_user.id)
		if not auth:
...
async def approve_pm(client, message):
	...
	async with lock:
		if message.chat.type == 'private':
			rart = get_authorized(message.chat.id)
			if rart:
				rart.approved = True
			else:
...
async def pm_button(client, query):
	...
	async with lock:
		auth = get_authorized(query.from_user.id)
	...
	if subcommand == "block":
		...
		pm_bot_message = f'{_make_sexy(query.from_user)} wants to contact {_make_sexy(me)}'
		await slave.send_message(config['config']['log_chat'], pm_bot_message, reply_markup=buttons)
		auth.requested = True
		async with lock:
			session.commit()
	elif subcommand == "apr":
		...
		async with lock:
			...
			if auth:
				session.delete(auth)
				session.commit()
		await query.message.edit_text("User blocked and reported.")
